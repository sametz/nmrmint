...
class BaseEntryFrame(Frame):
    """
    What are the must-haves for an entry frame?
    objects:
    -a label (could be overwritten if not wanted)
    -a data entry type (default: tkinter.Entry)
    -a StringVar object associated with the entry
    ...
    behavior:
    -bindings (default: update on change, then key behavior)
    -validation
    -widget traversal

    What may differ?
    -the type of Entry widget
    -the type of validation
    -value storage
    -args/kwargs for callback
    """
...
class EntryFrame(Frame):
    ...
    EntryFrame will call the function (provided as 'controller' in the arguments)
    ...
    TODO:
        * 'controller' is a misleading argument name when a MVC design is used.
...
class ArrayBox(BaseEntryFrame):
    ...
    def __init__(self, parent=None,
        ...
        BaseEntryFrame.__init__(self, parent,
                            # name, color,
                            **options)
...
class ArraySpinBox(ArrayBox):
    ...
    Arguments (in addition to standard ArrayBox options):
        ...
        realtime: True if data/controller should be refreshed as the SpinBox arrow
                  button is held down.
    """
    def __init__(self, parent=None, from_=0.00, to=100.00, increment=1,
        ...
        :param realtime: (boolean) True if data/controller should be refreshed as
...
class IntBox(Frame):
    ...
    def on_event(self, event):
        ...
        to send data to the controller, and shifts focus to the next entry box (after
        Return or Tab).
        """
        self.to_dict()
        self.master.request_plot()
...
class FloatEntry(Entry):
    """Extends tkinter Entry to limit entry to float type"""
if __name__ == '__main__':
    ...
    class DummyFrame(Frame):
        ...
        def request_plot(self):
            ...
            # print('requesting calculation from the controller')
            # print(dummy_array)
