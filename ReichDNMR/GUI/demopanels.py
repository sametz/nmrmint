...
class SeeDismissPanel(ttk.Frame):
    def __init__(self, master):
        ...
        dismissBtn = ttk.Button(text='Dismiss', #image=imh,
                                command=self.winfo_toplevel().destroy)
        #dismissBtn.image = imh      # prevent image from being garbage
        # collected
        ...
        codeBtn = ttk.Button(text='See Code', #image=imh,
                             default=ACTIVE,
...
class CodeDialog(Dialog):
    ...
    def body(self, master):
        """Overrides Dialog.body() to populate the dialog window with a scrolled
        text window and custom dialog buttons. """
        ...
        text = Text(txtFrame, height=24, width=100, wrap=WORD, setgrid=1,
    ...
    def buttonbox(self):
        """Overrides Dialog.buttonbox() to create custom buttons for this
        dialog. """
